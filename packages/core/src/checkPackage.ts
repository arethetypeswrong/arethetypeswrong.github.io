...
import type { Host, Analysis, FS, ResolutionKind, EntrypointResolutionAnalysis, TraceCollector, Resolution } from './types.js';

export async function checkPackage(packageName: string, packageVersion?: string, host: Host = fetchTarballHost): Promise<Analysis> {
...
function checkEntrypoints(packageName: string, fs: FS): Record<string, Record<ResolutionKind, EntrypointResolutionAnalysis>> {
...
function checkEntrypointTyped(packageName: string, fs: FS, resolutionKind: ResolutionKind, entrypoint: string): EntrypointResolutionAnalysis {
