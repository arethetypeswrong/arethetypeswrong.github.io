...
export type ProblemSummary =
  | NoTypesProblemSummary
  | ResolutionProblemSummary;
...
export function getProblems(result: Analysis): Problem[] {
  ...
  for (const subpath in result.entrypointResolutions) {
    ...
    for (const kind in entrypoint) {
      ...
      if (resolution?.moduleKind === ts.ModuleKind.ESNext && implementationResolution?.moduleKind === ts.ModuleKind.CommonJS) {
        problems.push({
          kind: 'FalseESM',
          entrypoint: subpath,
          resolutionKind,
      ...
      else if (resolution?.moduleKind === ts.ModuleKind.CommonJS && implementationResolution?.moduleKind === ts.ModuleKind.ESNext) {
        problems.push({
          kind: 'FalseCJS',
          entrypoint: subpath,
          resolutionKind,
...
export function groupResolutionProblems(problems: ResolutionProblem[]): Partial<Record<ResolutionProblemKind, ResolutionProblem[]>> {
  ...
  for (const kind in grouped) {
    ...
    const summary: ResolutionProblemSummary = {
      ...
      ...getMessage(
        ...
        Array.from(affectedResolutionKinds),
        analysis)
    };
    result.push(summary);
  }
...
function getMessage(
  ...
  const affectsAllNode16 = affectsAllResolutionKinds ||
    affectedResolutionKinds.includes('node16-cjs') &&
    affectedResolutionKinds.includes('node16-esm');
  const affectsSomeNode16 =
    affectedResolutionKinds.includes('node16-cjs') ||
    affectedResolutionKinds.includes('node16-esm');
  return msg(f => {
    const entrypoints =
      allEntrypoints.size === 1 ? 'the package' :
      affectsAllEntrypoints ? f.strong('all entrypoints') :
      affectedEntrypoints.length === 1 ? `the ${
        f.code(`"${formatEntrypoint(analysis.packageName, affectedEntrypoints[0])}"`)
      } entrypoint` :
      f.strong(`${affectedEntrypoints.length} entrypoints`);
    
    const affectsSingleResolutionSetting = affectedResolutionKinds.length === 1 ||
      affectedResolutionKinds.length === 2 && affectsAllNode16;
    ...
    if (affectsSingleResolutionSetting) {
      ...
      if (affectsAllNode16 || !affectsSomeNode16) {
        resolutionSettings = ' when compiled under the '
        ...
        resolutionSettings = ' when compiled under the '
          + affectedResolutionKinds.slice(0, -1).map(k => f.code(moduleResolutionKinds[k])).join(', ')
          + `${needsComma ? ',' : ''} or ${formatResolutionKind(affectedResolutionKinds[affectedResolutionKinds.length - 1], f)}`;
      }
    }
    ...
    switch (kind) {
      ...
      case 'CJSResolvesToESM':
        return `Imports of ${entrypoints} only resolved to ${affectedEntrypoints.length === 1 ? 'an ES module' : 'ES modules'} `
...
function formatResolutionKind(kind: ResolutionKind, f: Formatters) {
  switch (kind) {
    ...
    case 'node16-cjs':
      return `the ${f.code('node16')} module resolution setting when the importing module is CJS `
    ...
    case 'node16-esm':
      return `the ${f.code('node16')} module resolution setting when the importing module is ESM `
