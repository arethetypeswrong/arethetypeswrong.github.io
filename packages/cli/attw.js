...
async function typed(analysis, opts) {
  ...
  const entrypoints = subpaths.map((s) => {
    ...
    if (s === ".") return chalk2.bold[color](`"${analysis.packageName}"`);
    else return chalk2.bold[color](`"${analysis.packageName}/${s.substring(2)}"`);
  });
  ...
  allResolutionKinds.forEach((kind) => {
    ...
    row = row.concat(
      subpaths.map((subpath) => {
        ...
        const moduleResult =
          (!opts.emoji ? "OK " : "\u{1F7E2} ") +
          moduleKinds[
          ((_a = resolution == null ? void 0 : resolution.moduleKind) == null ? void 0 : _a.detectedKind) || ""
          ];
...
${error.message}`,
    ...
    if (opts.raw) {
      ...
      if (opts.strict && analysis.containsTypes && !!core2.getProblems(analysis).length) process.exit(1);
      return;
    }
    console.log();
    if (analysis.containsTypes) {
      await typed(analysis, opts);
      if (opts.strict && !!core2.getProblems(analysis).length) process.exit(1);
    } else {
      untyped(analysis);
    }
  })
