...
).argument("<package-name>", "the package to check; by default the name of an NPM package, unless --from-file is set").option("-f, --from-file", "read from a file instead of the npm registry").option("-s, --strict", "exit if any problems are found (useful for CI)").option("-v, --package-version <version>", "the version of the package to check").option("-q, --quiet", "don't print anything to STDOUT (overrides all other options)").option("-r, --raw", "output raw JSON; overrides any rendering options").option("-E, --vertical", "display in a vertical ASCII table (like MySQL's -E option)").option("-F, --flipped", "flip the table (so the resolution kinds are on top and the entry points are on the side)").option("--summary, --no-summary", "whether to print summary information about the different errors").option("--emoji, --no-emoji", "whether to use any emojis").option("--color, --no-color", "whether to use any colors (the FORCE_COLOR env variable is also available)").option("--config-path <path>", "path to config file (default: ./.attw.json)").addOption(new Option("-i, --ignore <rules...>", "specify rules to ignore").choices(Object.values(problemFlags))).action(async (packageName) => {
  var _a;
